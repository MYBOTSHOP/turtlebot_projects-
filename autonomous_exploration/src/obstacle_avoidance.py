...
def start(msg, x, y):
	...
	# Map matrix with values in meters
	maps = {} 
	maps_copy = {}
	for i in angle:
		if round(float(msg.ranges[i]),2) == 0:
...
def control(maps2,distance_differ,mod,k):
	...
	while(bandiera):	
		...
		for i in range(point):
			p = p_start - point/2 + i
			space_angel.append(angle[p])
		value =distance_differ[z]
		for j in space_angel:
			if maps2[j] >= value:
				free_space = 1
			else:
				free_space = 0
...
def slam(map_matrix,maps,init,odometryx,odometryy,odometryw):
	...
	for i in range(len(maps)/2):
		...
		if maps_value >0  and maps_value <5 and time % 10 == 0:
			...
			index[1] += int(y*100 + maps_value*100*math.cos((theta+i)*math.pi/180))
...
def LiDAR(msg):
	...
	if cnt == 1:   
		...
		print("yaha error he",w)
		if w>1:
			w=1
		new_w = numpy.sign(z)*round(float(2*math.acos(w)*180/math.pi), 2)          
		...
		odometryw = numpy.concatenate((odometryw, [new_w] ))   
